...
const EMPTY = { name: "", format: "", season: "", start_date: "" };
...
export default function TournamentsPage() {
  ...
  const load = () => {
    setLoading(true);
    fetchTournaments().then(setItems).catch((e) => setError(e.message)).finally(() => setLoading(false));
  };
  useEffect(load, []);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-[#0d1424] border border-white/5 rounded-xl p-5">
        ...
        <form onSubmit={add} className="grid sm:grid-cols-2 lg:grid-cols-5 gap-3">
          <Field ph="Name *" val={form.name} on={(v) => setForm({ ...form, name: v })} w="lg:col-span-2" />
          ...
          <button type="submit" disabled={saving}
            className="lg:col-span-5 sm:w-auto px-4 py-2.5 bg-gradient-to-r from-falcon-gold to-amber-600 text-white font-semibold rounded-xl hover:shadow-lg transition-all disabled:opacity-50 flex items-center justify-center gap-2">
            ...
            <div key={t.id} className="bg-[#0d1424] border border-white/5 rounded-xl p-4">
              {editId === t.id ? (
                <div className="grid sm:grid-cols-2 lg:grid-cols-5 gap-2 items-center">
                  <Field ph="Name *" val={edit.name} on={(v) => setEdit({ ...edit, name: v })} w="lg:col-span-2" />
