...
export default function UmpiringPage() {
  ...
  const [edit, setEdit] = useState<{ duty_date: string; umpire1: string; umpire2: string; notes: string }>({
    duty_date: "", umpire1: "", umpire2: "", notes: "",
  });
  ...
  const load = () => {
    setLoading(true);
    Promise.all([fetchAllProfiles(), fetchDuties(), fetchPendingSwaps()])
      .then(([p, d, s]) => { setProfiles(p); setDuties(d); setSwaps(s); })
  ...
  const addDuty = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      await createDuty({ duty_date: date, umpire1: ump1 || null, umpire2: ump2 || null, notes: notes.trim() || null });
      setDate(""); setUmp1(""); setUmp2(""); setNotes("");
      load();
    } catch (err) {
  ...
  const startEdit = (d: Duty) => {
    setEditId(d.id);
    setEdit({ duty_date: d.duty_date, umpire1: d.umpire1 ?? "", umpire2: d.umpire2 ?? "", notes: d.notes ?? "" });
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-[#0d1424] border border-white/5 rounded-xl p-5">
        ...
        <form onSubmit={addDuty} className="grid sm:grid-cols-2 lg:grid-cols-5 gap-3 items-center">
          <input
            type="date"
            value={date}
                  ...
                  {editId === d.id ? (
                    <div className="grid sm:grid-cols-2 lg:grid-cols-5 gap-2 items-center">
                      ...
                      <input value={edit.notes} onChange={(e) => setEdit({ ...edit, notes: e.target.value })} placeholder="Note"
                        className="px-2 py-1.5 bg-white/5 border border-white/10 rounded-lg text-xs text-falcon-cream focus:outline-none focus:border-falcon-gold/40" />
                      <div className="flex gap-2">
                        <button onClick={() => saveEdit(d.id)} className="p-1.5 rounded-lg bg-emerald-500/15 text-emerald-300"><Check className="w-4 h-4" /></button>
