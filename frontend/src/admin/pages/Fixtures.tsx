...
import {
  ...
  fetchFixtureAvailability, setMatchAvailability,
  type Fixture, type MatchAvailability, type MatchAvailStatus,
} from "@/lib/db";

const STATUSES: { key: MatchAvailStatus; label: string; cls: string; active: string }[] = [
  { key: "available",   label: "Available",   cls: "text-emerald-400", active: "bg-emerald-500/20 border-emerald-500/40 text-emerald-300" },
  { key: "maybe",       label: "Maybe",       cls: "text-amber-400",   active: "bg-amber-500/20 border-amber-500/40 text-amber-300" },
  { key: "unavailable", label: "Unavailable", cls: "text-red-400",     active: "bg-red-500/15 border-red-500/40 text-red-300" },
];
...
const EMPTY = { tournament: "", opponent: "", match_date: "", match_time: "", ground: "", format: "", notes: "" };
...
export default function FixturesPage() {
  ...
  // expanded fixture availability
  const [openId, setOpenId] = useState<string | null>(null);
  const [avail, setAvail] = useState<Record<string, MatchAvailability[]>>({});

  const load = () => {
    setLoading(true);
    fetchFixtures().then(setFixtures).catch((e) => setError(e.message)).finally(() => setLoading(false));
  };
  useEffect(load, []);

  const toForm = (f: Fixture) => ({
    tournament: f.tournament, opponent: f.opponent ?? "", match_date: f.match_date,
    match_time: f.match_time?.slice(0, 5) ?? "", ground: f.ground ?? "", format: f.format ?? "", notes: f.notes ?? "",
  });

  const clean = (v: typeof EMPTY) => ({
    tournament: v.tournament.trim(),
  ...
  const add = async (e: React.FormEvent) => {
    e.preventDefault();
    setError("");
    if (!form.tournament.trim() || !form.match_date) return setError("Tournament and date are required.");
    setSaving(true);
    try {
      await createFixture(clean(form));
      setForm({ ...EMPTY });
      load();
    } catch (err) {
      setError(err instanceof Error ? err.message : "Could not add fixture.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const saveEdit = async (id: string) => {
    if (!edit.tournament.trim() || !edit.match_date) return setError("Tournament and date are required.");
  ...
  const Field = ({ k, ph, type = "text", val, on }: { k: string; ph: string; type?: string; val: string; on: (v: string) => void }) => (
    ...
    <div className="space-y-6">
      ...
      <div className="bg-[#0d1424] border border-white/5 rounded-xl p-5">
        ...
        <form onSubmit={add} className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
          <Field k="tournament" ph="Tournament *" val={form.tournament} on={(v) => setForm({ ...form, tournament: v })} />
          <Field k="opponent" ph="Against (opponent)" val={form.opponent} on={(v) => setForm({ ...form, opponent: v })} />
          <Field k="ground" ph="Ground / venue" val={form.ground} on={(v) => setForm({ ...form, ground: v })} />
          <Field k="match_date" ph="Date" type="date" val={form.match_date} on={(v) => setForm({ ...form, match_date: v })} />
          <Field k="match_time" ph="Time" type="time" val={form.match_time} on={(v) => setForm({ ...form, match_time: v })} />
          <Field k="format" ph="Format (e.g. T20)" val={form.format} on={(v) => setForm({ ...form, format: v })} />
          <input value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} placeholder="Notes (optional)"
            className="sm:col-span-2 px-3 py-2.5 bg-white/5 border border-white/10 rounded-lg text-sm text-falcon-cream placeholder:text-falcon-cream/30 focus:outline-none focus:border-falcon-gold/40" />
          <button type="submit" disabled={saving}
            className="px-4 py-2.5 bg-gradient-to-r from-falcon-gold to-amber-600 text-white font-semibold rounded-xl hover:shadow-lg transition-all disabled:opacity-50 flex items-center justify-center gap-2">
      ...
      {loading ? (
        ...
        <p className="text-falcon-cream/30 text-sm bg-[#0d1424] border border-white/5 rounded-xl p-6 text-center">No fixtures yet. Add one above.</p>
      ) : (
              ...
              <div key={f.id} className="bg-[#0d1424] border border-white/5 rounded-xl overflow-hidden">
                {editId === f.id ? (
                  <div className="p-4 grid sm:grid-cols-2 lg:grid-cols-3 gap-2">
                    <Field k="t" ph="Tournament *" val={edit.tournament} on={(v) => setEdit({ ...edit, tournament: v })} />
                    <Field k="o" ph="Against" val={edit.opponent} on={(v) => setEdit({ ...edit, opponent: v })} />
                    <Field k="g" ph="Ground" val={edit.ground} on={(v) => setEdit({ ...edit, ground: v })} />
                    <Field k="d" ph="Date" type="date" val={edit.match_date} on={(v) => setEdit({ ...edit, match_date: v })} />
                    <Field k="tm" ph="Time" type="time" val={edit.match_time} on={(v) => setEdit({ ...edit, match_time: v })} />
                    <Field k="fmt" ph="Format" val={edit.format} on={(v) => setEdit({ ...edit, format: v })} />
                  ...
                  <div className="flex items-center gap-4 p-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-falcon-cream/40 truncate">
                        {[f.ground, f.format, f.notes].filter(Boolean).join(" · ")}
                      </div>
                    </div>
